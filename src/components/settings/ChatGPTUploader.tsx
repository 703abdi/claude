...
export default function ChatGPTUploader() {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const res = await fetch("/api/connectors/chatgpt/upload", {
        method: "POST",
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted">
        ChatGPT has no public API for a third party to pull your conversation history automatically. Export it
        yourself (Settings → Data controls → Export in ChatGPT), then upload the <code>conversations.json</code>{" "}
        file here.
      </p>
      <input
        ref={inputRef}
