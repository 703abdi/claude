 ...
 * sync agent's and ChatGPT connector's contract. Anthropic has no live API
 * that streams a user's own Claude conversation history to a third party
 * today, so nothing calls this route automatically yet — the working path
 * is the session-authenticated /api/connectors/claude/upload endpoint fed
 * by Claude.ai's manual data export. This route exists so a future
 * automated bridge (should Anthropic ever expose one, or a local
 * export-watcher script) can push data in without any other code changing.
 */
...
export async function POST(req: NextRequest) {
  if (!verifyBearerToken(req, "CLAUDE_CONNECTOR_TOKEN")) {
