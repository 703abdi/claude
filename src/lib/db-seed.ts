...
import { TaskStatus, Effort, ContextSourceType, ContextType, SuggestionType } from "@prisma/client";
...
async function ensureAdminUser(prisma: PrismaClient) {
  ...
  if (!email || !password) {
    return { ok: false, message: "ADMIN_EMAIL / ADMIN_PASSWORD not set — skipped admin user creation." };
  }
  ...
  return { ok: true, message: `Admin user ready: ${email}` };
}
...
async function ensurePeople(prisma: PrismaClient) {
  const people = [
...
/**
 * Idempotent: creates/updates the admin user from ADMIN_EMAIL/ADMIN_PASSWORD
 * every time, but only seeds demo data once (skips if any isSeed task
 * already exists). Shared between the `db:seed` CLI script and the
 * one-time /api/setup HTTPS endpoint (useful when a sandbox or CI
 * environment can't reach the database directly on its raw TCP port —
 * only the deployed app's own HTTPS endpoint can).
 */
export async function runSeed(prisma: PrismaClient) {
  const log: string[] = [];
  const admin = await ensureAdminUser(prisma);
  log.push(admin.message);

  const seedFlag = await prisma.task.findFirst({ where: { isSeed: true } });
  if (seedFlag) {
    log.push("Seed demo data already present — skipped (admin user still synced above).");
    return { adminReady: admin.ok, seeded: false, log };
  }

  const cat = await ensureCategories(prisma);
  const person = await ensurePeople(prisma);
  ...
  await prisma.taskDependency.create({ data: { blockerId: t2.id, blockedId: t7.id } });
  ...
  return { adminReady: admin.ok, seeded: true, log };
}
